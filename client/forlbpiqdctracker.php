...
<!-- <form method="post">
  ...
  <?php 
        ...
        if ($Payment_Status == 'Paid') {
          ...
          echo '<form id="paymentForm" method="POST" class="form-container" hidden>';
...
<?php
  ...
  WHERE lumber_app_id = $lumber_app_id && Title = 'Credit  Officer' ";
  $stmt = $connection->prepare($sql);
  $stmt->execute(array(
  

  ':Title' => 'Credit  Officer (Successfully Paid)',
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if ($response === false) {
    ...
    echo $first_part; // Output: 00|0000
        // $first_part = "00|0000";
   ...
   function function_alert($message) {
     ...
     // echo "<script type='text/javascript'>alert('Successfully Rated'); window.location.href=docstatus_released.php?lumber_app_id="$lumber_app_id";</script>";
      ...
      function_alert("You have already paid successfully!");
        ...
        echo "<script type='text/javascript'>alert('Payment Transaction Pending Please Wait...'); window.close();</script>";
      ...
      // echo "<script>alert('Bills are Unpaid'); window.location.href = 'application.php';</script>";

      // echo "<script>window.location.href='doctracker.php?lumber_app_id=" . $selectedRowId . "';</script>";
