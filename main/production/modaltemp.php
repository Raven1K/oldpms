...
<div class="modal-dialog modal-xl">
  <div class="modal-content">
    ...
    <!-- <button name="submit" type="button" class="close" data-dismiss="modal" onclick="redirectToPage()">&times;</button> -->
    <a href="application.php" class="close"  id="exampleModalhide2">&times;</a>
...
$tatalcontvol = $_POST['result'];
$other = (isset($_POST['other']) ? $_POST['other'] : '') . ' ' . (isset($_POST['others']) ? $_POST['others'] : '');
...
$checkQuery = $connection->prepare("SELECT COUNT(*) FROM supp_contdetails WHERE lumber_app_id = :lumber_app_id");
...
$query->bindParam("mang", $mang, PDO::PARAM_STR);
...
<h2 class="modal-title text-warning"><strong>Add Supplier Contract Details:</strong></h2>
...
<div class="col-md-12 col-sm-12  form-group has-feedback">
  <input type="text" class="form-control has-feedback-left" placeholder="Supplier Business Trade Name" name="bname" id="bname" value="<?php //echo $lumber_ap_row["Name_of_Permitte"];?>" >
    <span class="fa fa-user form-control-feedback left" aria-hidden="true"></span>
</div>
<div class="col-md-12 col-sm-12  form-group has-feedback">
  <input type="text" class="form-control has-feedback-left" required="required"  placeholder="Plantation Address" name="Saddress" id="Saddress" value="<?php //echo $lumber_ap_row1["full_address"];?>">
    <span class="fa fa-user form-control-feedback left" aria-hidden="true"></span>
</div>
<div class="col-md-12 col-sm-12  form-group has-feedback">
  <input type="text" class="form-control has-feedback-left" required="required"  placeholder="Supplier Owner Name" name="ownername" id="ownername" value="<?php //echo $lumber_ap_row["Name_of_Permitte"];?>" >
    <span class="fa fa-user form-control-feedback left" aria-hidden="true"></span>
</div>
<div class="col-md-12 col-sm-12  form-group has-feedback">
<input type="text" class="form-control has-feedback-left" placeholder="Total Contracted Volume" name="result" id="result" hidden >
    <span class="fa fa-user form-control-feedback left" aria-hidden="true"></span>
</div>
<div class="col-md-12 col-sm-12  form-group has-feedback">
<!--  <label type="text" class="control-label has-feedback-left"  required="required" placeholder="PTPOC/PTPR Holder" name="ptpoc" id="ptpoc"></label>-->
  ...
  <option value="CTPO">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Certificate of Tree Plantation Ownership(CTPO)</option>
  <option value="CBFM">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Community Based Forest Management Agreement</option>
  <option value="WPP">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Wood Processing Plant Permit(WPP)</option>
	</select>
</div>
<div class="col-md-12 col-sm-12  form-group has-feedback">
  <input type="text" class="form-control has-feedback-left"  required="required" placeholder="PTPOC/PTPR Number" name="ptadd" id="ptadd">
  <span class="fa fa-user form-control-feedback left" aria-hidden="true"></span>
</div>
...
<div class="col-md-8 col-sm-8  form-group has-feedback">
  <input type="date" class="form-control input"  required="required" placeholder="Expiration Date" name="exdate" id="exdate" date_format="m/d/Y" required>
</div>
	
<div class="col-md-12 col-sm-12  form-group has-feedback">
  <input type="number" class="form-control has-feedback-left"  placeholder="# Year of Validity" name="validity_val" id="ptadd" required>
  <span class="fa fa-user form-control-feedback left" aria-hidden="true"></span>
</div>
<!--
...
<div class="form-group row">
  <div class="col-md-5 col-sm-5  ">
    <button type="submit" class="btn btn-success" name="Submit_supplier">Add Supplier</button>
    <!-- <a type="button" class="btn btn-success" href="add_wood_species_endorse.php?lumber_app_id=<?php echo $l_id; ?>" name="Submit_supplier">Add Supplier</a> -->
  </div>
...
<script>
        $(document).ready(function(){
            $(".input").keyup(function(){
                var val1 = +($("#falcu").val() || 0);
                var val2 = +($("#macu").val() || 0);
                var val3 = +($("#gecu").val() || 0);
                var val4 = +($("#cacu").val() || 0);
                var val5 = +($("#mancu").val() || 0);
                var val6 = +($("#mang").val() || 0);
                var val7 = +($("#others").val() || 0);
                
                // Update the result field
                $("#result").val(val1 + val2 + val3 + val4 + val5 + val6 + val7);
            });
        });
    </script>
...
<div class="col-md-12 col-sm-12">    
  ...
  <?php 
    ...
    echo '<td style="text-align: center"><div class="dropdown">
...
<div class="modal-dialog modal-lg">
  <div class="modal-content">
	<div class="modal-header">
		<h5 class="modal-title">Mayor's Permit Details</h5>
    ...
    <div class="container-fluid">
	  ...
	  <div class="row">
		  <div class="col-sm-12">
		   									...
		   									<div class="item form-group row">
												<div class="col-md-5 col-sm-5 ">
													<input type="text" class="form-control" Date name="bnissued" id="bnissued" name="bnissued" value="bnissued" hidden>
												</div>
											</div>
			  								<div class="item form-group row">
												<div class="col-md-5 col-sm-5 ">
													
                          <input type="date" class="form-control" name="dtissued" id="dtissued" value="<?php echo date('Y-m-d'); ?>" hidden>

												</div>
											</div>
			  								<div class="item form-group row">
												<div class="col-md-5 col-sm-5 ">
													<input type="date" class="form-control"  name="dtiexpiry" id="dtiexpiry" value="<?php echo date('Y-m-d'); ?>" hidden >
												</div>
											</div>
		  </div>
	</div>
...
//   // The code to execute when the button is clicked
...
//   echo '<script>alert("Successfully Forwarded to Chief RPS");</script>';
...
if (isset($_POST['update_button'])) {
  // Fetch the 'result' field from supp_contdetails where lumber_app_id matches
  $check_sql = "SELECT result FROM supp_contdetails WHERE lumber_app_id = :lumber_app_id";
  $check_stmt = $connection->prepare($check_sql);
  $check_stmt->execute(array(':lumber_app_id' => $lumber_app_id));
  $result = $check_stmt->fetch(PDO::FETCH_ASSOC);

  if ($result && $result['result'] === '0') {
      // The 'result' field has a value of '0', display an alert
      echo "<script>alert('Cannot forward to RPS, please add species. Action Button Select Add Species');</script>";
  } else {
      // 'result' field is not '0', proceed with the update
      $stat_uss = 'For Initial Chief RPS';
      $Flow_stats = '7';
      ...
      echo "<script>alert('Sucessfully Forwarded to RPS');</script>";
  }
}
