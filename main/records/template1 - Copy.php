...
<table border="7">
	...
	<table border="0">
		<tr>
		 ...
		 <td style="width: 20%;" style="background-image:url('records.png'); background-size: 200px 90px; background-repeat: no-repeat;"><p style="text-align: left; padding-top: 48px; padding-left: 65px; line-height: 10px; font-size: 10px"> {{date}} </br>
					  {{LD-random}}-{{signature}}
...
// while ( $row = $stmt->fetch(PDO::FETCH_ASSOC) ) {
            ...
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
	            echo '<tr class="item">';
	            echo '<td style="text-align: left"><u>'.$row['ownername'].' </u></td>';
	            echo '<td style="text-align: left"><u>{{volume}} board feet of Chainsaw-cut lumbers of {{treespecie}}</u></td>';
	            echo '</tr>';
            }
			?>
        </tbody>
	...
	<p style="text-align:justify; font-size: 15px">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Issued on <u><strong>{{date}}</strong></u> at <u><strong>Ambago, Butuan City, Agusan del Norte, Philippines</strong></u>.</p>
<div id="example2">	
